...
// ======> CONSTANTES <======
#define LED_INDICADOR 2 // led de la ESP32 que indica bloqueo de la entrada
#define SENSOR_ENTRADA 21
#define SENSOR_SALIDA 22
#define MAX_CARROS 6

#define POS_CERRADO 20 // posición del servo cuando está cerrrado
#define POS_ABIERTO 110 // posición del servo cuando está abierto

#define TIEMPO_ABIERTA 5000 // 5 segundos de apertura
...
// ======> MULTI-WIFI: 6 redes guardadas <======
const char* lista_ssid[6] = {"EB_WiFiB", "EB_WiFi", "Bryan Alexis's Galaxy A33 5G", "POCO C65", "Wi-Fi IPN", "INFINITUM4776_2.4"};
const char* lista_pass[6] = {"FamEB2023?", "FamEB2023?", "vtwv7692", "jasonmomoa", "", "MM980321ze"};

// ======> VARIABLES <======
bool bloqueo_manual = false; // activado desde Blynk
bool bloqueo_auto = false; // activado al llenarse el estacioanmiento
...
void conectarWifiMulti() {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    while (millis() - inicio < 6000) { // 6 segundos por red
...
/*
 * Actualiza el bloqueo general dependiendo del bloqueo automático y del manual.
 */
...
void controlar_estacionamiento() {
  ...
  // ---------- ENTRADA ----------
  if (lectura_entrada == LOW) contador_entrada++;
  else contador_entrada = 0;
  ...
  // ---------- SALIDA ----------
  if (lectura_salida == LOW) contador_salida++;
  else contador_salida = 0;
...
// ========== BLOQUEO MANUAL DESDE BLYNK ==========

/*
 * Recibe la instrucción desde Blynk para el bloqueo manual de la entrada. 
 */
BLYNK_WRITE(V6) {
...
void loop() {
  if (!Blynk.connected()) { // si no esta conectado
    conectarWifiMulti();
  }
